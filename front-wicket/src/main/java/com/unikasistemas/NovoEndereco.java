...
public class NovoEndereco extends BasePage{
    private final MonitoradorServico servico = new MonitoradorServico();
    
    public NovoEndereco(final Monitorador monitorador){
        ...
        CompoundPropertyModel<Endereco> compoundPropertyModelEndereco = new CompoundPropertyModel<Endereco>(new Endereco());
        ...
        AjaxButton botaoSubmit = new AjaxButton("botaoSubmit", form) {
            ...
            @Override
            protected void onSubmit(AjaxRequestTarget target, Form<?> formulario) {
                Endereco e = form.getModelObject();
                e.setMonitorador(monitorador);
                e.setPrincipal(selected.getObject());

                Endereco response = servico.adicionarEndereco(e);
                System.out.println("Endereco: " + response);
        ...
        form.add(botaoVoltar, botaoSubmit);
    }
}
